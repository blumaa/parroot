...
import { Box, Button, Heading, Text, Card, Spinner } from '@mond-design-system/theme';
...
const SEGMENT_TYPES = [
  { type: 'carousel', label: 'Carousel', description: 'Rotating image or content slider', icon: '🎠' },
  { type: 'text-block', label: 'Text Block', description: 'Rich text content section', icon: '📝' },
  { type: 'gallery', label: 'Gallery', description: 'Grid of images or media', icon: '🖼️' },
  { type: 'cta', label: 'Call to Action', description: 'Prominent call-to-action section', icon: '📣' },
  { type: 'form', label: 'Form', description: 'Contact or inquiry form', icon: '📧' },
  { type: 'posts', label: 'Posts', description: 'A collection of posts', icon: '📰' },
] as const;

export function SegmentDrawer() {
  const { isSegmentDrawerOpen, closeSegmentDrawer, editingSegmentId } = useSiteBuilder();
  const { data: existingSegment, isLoading: isLoadingSegment } = useSegment(editingSegmentId);
  ...
  return (
    <Drawer isOpen={isSegmentDrawerOpen} onClose={handleClose}>
      <DrawerHeader onClose={handleClose}>
        <Heading level={3} size="lg">
          {editingSegmentId ? 'Edit Segment' : showForm ? 'Create Segment' : 'Choose Segment Type'}
        </Heading>
      </DrawerHeader>
              ...
              <Card key={segmentType.type}>
                <Box padding="3" display="flex" flexDirection="column" gap="xs">
                  <Box display="flex" alignItems="center" gap="sm">
                    ...
                    <Heading level={4} size="sm">{segmentType.label}</Heading>
                  </Box>
                  ...
                  <Box>
                    <Button
                      variant="primary"
                      size="sm"
                      onClick={() => setSelectedType(segmentType.type as SegmentType)}
                    >
